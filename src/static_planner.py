...
from geometry import LineSegment, Path, Point, Interval, Velocity
...
class StaticPlanner:
    ...
    def __init__(self, scene):
        """
        Creates a global static path planner for the scene.
        ...
        for pedestrian in self.scene.pedestrian_list:
            goal = random.sample(self.scene.exit_set, 1)[0]
            pedestrian.path = self.create_path(pedestrian, goal)
    ...
    def collective_update(self):
        ...
        for pedestrian in self.scene.pedestrian_list:
            if not stationary_pedestrian_array[pedestrian.counter] and self.scene.alive_array[pedestrian.counter]:
                ...
                pedestrian.path = self.create_path(pedestrian, pedestrian.goal)
                pedestrian.line = pedestrian.path.pop_next_segment()
                pedestrian.velocity = Velocity(pedestrian.line.end - pedestrian.position.array)
...
class GraphPlanner(StaticPlanner):
    ...
    def create_path(self, pedestrian: Pedestrian, goal_obstacle) -> Path:
        ...
        for point in path[1:-1]:
            ...
            prev_point = point
        finish_point = StaticPlanner.get_closest_goal_position(prev_point, goal_obstacle)
    ...
    def _fill_with_required_edges(self, node: Point, graph, goal_obstacle):
        ...
        :return:
        """
        goal_point = StaticPlanner.get_closest_goal_position(node, goal_obstacle)
